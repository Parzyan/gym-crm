...
public class TrainerWorkloadService {
    ...
    private static final Map<String, TrainerSummary> trainerData = new ConcurrentHashMap<>();

    public void updateWorkload(TrainerWorkloadRequest dto) {
        TrainerSummary summary = trainerData.computeIfAbsent(dto.getTrainerUsername(), username -> {
        ...
        int year = dto.getTrainingDate().getYear() + 1900;
        int month = dto.getTrainingDate().getMonth() + 1;
    ...
    public TrainerSummary getTrainerSummary(String username) {
        return trainerData.get(username);
    }
}
